...
class Function(object):
    ...
    def __init__(self, type_name, inputs, params):
        ...
        self._top_names = self.params.get('top_names', None)
        top_name = self.params.get('top_name', None)
        if top_name is not None:
            self._top_names = [top_name]
        if self._top_names is not None:
            assert self.ntop == len(self._top_names)
        if 'name' in self.params:
            del self.params['name']
        if 'top_names' in self.params:
            del self.params['top_names']
        if 'top_name' in self.params:
            del self.params['top_name']
    ...
    def _get_top_name(self, top, names, autonames):
        if top not in names:
            if self._top_names is not None:
                names[top] = self._top_names[self.tops.index(top)]
            else:
